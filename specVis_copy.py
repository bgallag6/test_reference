# -*- coding: utf-8 -*-
"""
Created on Thu Jul 26 15:46:29 2018
...
def onclick(event):
    ...
    if ixx > 1. and iyy > 1.:
        ...
        try:                                 
            ...
            M2_low = [0., 0.3, -0.01, 0.00001, -6.5, 0.05]  # use for paper
            M2_high = [0.002, 6., 0.01, 0.2, -4.6, 0.8]   
            #M2_low = [0., 0.3, 0., 0.00001, -6.5, 0.05]
            #M2_high = [0.002, 6., 0.01, 0.2, -4.6, 0.8] 
        ...
        rval_log = pearsonr(np.log10(m2_fit2), np.log10(s))[0]
        ...
        ax2.set_xlim(10**-4.5, 10**-1.3)  # normal
        ax2.set_ylim(10**-5, 10**0) 
        #ax2.set_xlim(10**-5.5, 10**-1.3)  # 36-hour
        #ax2.set_ylim(10**-7, 10**0) 
        ax1.scatter(ix, iy, s=200, marker='x', c='white', linewidth=2.5)
        for label in legend.get_lines():
                label.set_linewidth(2.0)  # the legend line width   
        print(r'$\chi^2$ = {0:0.2f}  |  rval = {1:0.2f}  | rval(log) = {2:0.2f}'.format(redchisqrM22, rval, rval_log))
    return ix, iy
