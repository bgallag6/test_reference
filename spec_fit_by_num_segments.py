# -*- coding: utf-8 -*-
"""
Created on Mon Jul 16 11:30:58 2018
...
pixmed = DATA[:,125,25] / Ex  # extract timeseries + normalize by exposure time
...
for n in range(len(num_seg)):
    ...
    if n_segments == 1:
        fit1 = fit
        ax1 = plt.subplot2grid((20,21),(0, 0), colspan=10, rowspan=10)
        plt.loglog(freqs, s)
        ...
        fit3 = fit
        ax2 = plt.subplot2grid((20,21),(0, 11), colspan=10, rowspan=10)
        plt.loglog(freqs, s)
        plt.loglog(freqs, fit)
        plt.xlim(10**-5, 10**-1)
        plt.ylim(10**-5.5, 10**0.5) 
        plt.text(10**-4,10**-0.5,'n=%0.3f' % params[1])
        #avg_array3 = avg_array
        #avg_array3 = spec_geo/3.
    elif n_segments == 6:
        fit6 = fit
        ax3 = plt.subplot2grid((20,21),(11, 0), colspan=10, rowspan=10)
        plt.loglog(freqs, s)
        plt.loglog(freqs, fit)
        plt.xlim(10**-5, 10**-1)
        plt.ylim(10**-5.5, 10**0.5) 
        ...
        fit12 = fit
        ax4 = plt.subplot2grid((20,21),(11, 11), colspan=10, rowspan=10)
        plt.loglog(freqs, s)
        plt.loglog(freqs, fit)
        plt.xlim(10**-5, 10**-1)
        plt.ylim(10**-5.5, 10**0.5) 
